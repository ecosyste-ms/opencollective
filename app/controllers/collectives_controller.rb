class CollectivesController < ApplicationController
  ...
  def batch
    ...
    projects_scope = Project.joins(:collective).where(collectives: {id: @collectives})

    @pagy, @projects = pagy(projects_scope.order_by_stars)
    @transactions = Transaction.where(collective: @collectives).created_after(@start_date).any?
  end
